...
@interface FLTQuerySnapshotChangesStreamHandler ()
@property(readwrite, strong) id<FIRListenerRegistration> listenerRegistration;
@end

@implementation FLTQuerySnapshotChangesStreamHandler
...
- (FlutterError *_Nullable)onListenWithArguments:(id _Nullable)arguments
  ...
  id listener = ^(FIRQuerySnapshot *_Nullable snapshotChanges, NSError *_Nullable error) {
    if (error) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        NSMutableArray *toListResult = [[NSMutableArray alloc] initWithCapacity:2];

        NSMutableArray *documentChanges =
            [[NSMutableArray alloc] initWithCapacity:snapshotChanges.documentChanges.count];

        for (FIRDocumentChange *documentChange in snapshotChanges.documentChanges) {
          [documentChanges
        ...
        [toListResult addObject:[[FirestorePigeonParser
                                    toPigeonSnapshotMetadata:snapshotChanges.metadata] toList]];

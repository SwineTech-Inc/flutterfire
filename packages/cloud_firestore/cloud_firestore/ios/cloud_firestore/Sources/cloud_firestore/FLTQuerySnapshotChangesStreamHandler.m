...
- (FlutterError *_Nullable)onListenWithArguments:(id _Nullable)arguments
  ...
  id listener = ^(FIRQuerySnapshot *_Nullable snapshotChanges, NSError *_Nullable error) {
    if (error) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        // Emit the Pigeon object directly; the Pigeon-aware codec serializes the nested
        // `InternalDocumentChange` / `InternalSnapshotMetadata` with their proper type
        // codes. Pigeon 26 no longer flattens nested types via `toList`.
        events([FirestorePigeonParser toPigeonQuerySnapshotChanges:snapshotChanges
